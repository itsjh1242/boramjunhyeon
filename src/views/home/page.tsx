              ...
              <div className="relative flex flex-col" key={month}>
                ...
                {next && (
                  <div className="w-fit z-50 m-auto mt-[24px]">
                    <p
                      className="mb-4 px-4 py-2 border-[1px] rounded-full bg-white opacity-50"
                      onClick={() => handleInfiniteScrollNext()}
                    >
                      {nextMonth &&
                        `${parseInt(nextMonth.slice(4, 6), 10)}월 보기`}
                    </p>
                  </div>
                )}
              </div>
...
const MainSection: React.FC<{
  ...
  return (
    <section
      className={`relative w-full h-fit flex flex-col justify-start items-center px-[24px]`}
    >
      {/* 중앙 레이아웃 */}
      <div className="relative flex flex-col justify-start items-center w-full">
        ...
        <Separator
          ...
          className="absolute top-0 left-1/2 -translate-x-1/2 bg-black h-full w-[2px]"
        />
        {/* 중앙 텍스트 */}
        <p
          className={`w-fit h-fit p-4 rounded-full outline outline-1 bg-white shadow-lg font-bold z-10`}
        >
          ...
          Object.entries(item).map(([date, value], idx) => {
            ...
            return (
              <div
                ...
                className={`relative flex flex-col mt-4 w-[50%] ${
              ...
              >
                ...
                <div className="relative w-full flex items-center">
                  ...
                  <p
                    className={`w-fit max-w-fit text-gray-600 text-sm border-[1px] px-4 py-1 rounded-full border-gray-500 ${
                      right && "self-end"
                    }`}
                  >
                    {date.length === 8
                ...
                {value.location && (
                  ...
                  >
                    ...
                    <p
                      className={`text-gray-700 text-base font-medium break-words `}
                    >
                ...
                {/* 내용 */}
                <p className="mt-2 text-gray-800 text-sm break-words">
                ...
                <div className={`pr-2 py-4 ${right && "pl-2"}`}>
                  <div className="flex gap-2 overflow-x-auto">
                    {[...Array(10)].map((_, index) => (
                      <div
                        key={index}
                        className="min-w-[150px] h-[150px] bg-black text-white"
                      >
                        {_}dddddd
                      </div>
                    ))}
